const Textarea = ({ label, field, rows = 3, error, success, span, ...props }) => {
  return (
    <label className={`space-y-1.5 ${span || ''}`}>
      ...
      <textarea
        ...
        aria-invalid={!!error}
        className={`w-full resize-none rounded-xl border bg-surface px-4 py-3 text-sm text-on-surface outline-none transition-all focus:ring-2 focus:ring-secondary/20 ${error ? 'border-error focus:border-error' : success ? 'border-success focus:border-success' : 'border-outline-variant/25 focus:border-secondary'} ${props.disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
      />
