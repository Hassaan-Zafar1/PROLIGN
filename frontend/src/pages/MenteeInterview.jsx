            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
              {messages.map((m, i) => (
                <div key={i} className={`flex items-end gap-2 ayla-msg ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  ...
                  <div className={`max-w-[78%] rounded-2xl px-4 py-2.5 text-sm leading-relaxed ${m.role === 'user' ? 'bg-secondary text-on-secondary rounded-br-sm' : 'bg-surface-container-high text-on-surface rounded-bl-sm'
                    }`}>
                    {m.text}
                    {m.mode === 'voice' && (
